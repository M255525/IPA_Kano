...
- **互動式儀表板（單一 HTML）**：`滿意度分析互動儀表板_進階版.html` 為範例（本資料夾只保留進階版，基礎版已刪除）；八分頁（資料匯入／總覽洞察／IPA／Kano／整合優先序／柏拉圖／補充意見／匯出），全部本機運算、不上傳（唯一連網功能是選用的 AI 智慧摘要）。自動判斷 IPA 寬表格／Kano 寬表格／長表格。頂部另有跑馬燈與序號授權（鎖整個工具），見專案根目錄 `CLAUDE.md`。
