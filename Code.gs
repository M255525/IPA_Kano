/**
 * IPA_Kano 滿意度分析互動儀表板（進階版）—— 序號授權後端。
 * 貼到專用 Google Sheet 的「擴充功能 > Apps Script」，部署為 Web App 後，
 * 把取得的網址填進 滿意度分析互動儀表板_進階版.html 的 LICENSE_CHECK_URL。
 * 完整部署步驟見 SETUP-授權伺服器設定.md。
 *
 * 只做序號驗證，不代理任何付費 API（本工具沒有內建服務代理模式）。
 */
...
function checkOrActivate(serial) {
  ...
  try {
    ...
    // 若結束日期還沒算過（或開始日期是這次才補的），依開始日期 + VALID_AMOUNT（月）算出
    if (!endVal) {
